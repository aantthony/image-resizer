...
func fixExif(s image.Image, o int64) image.Image {
  ...
  for x := 0; x < dr.Max.X; x++ {
    for y := 0; y < dr.Max.Y; y++ {
      ...
      if (o == 1) { sColor = s.At(x, y);
      } else if (o == 2) { sColor = s.At(sr.Max.X - x, y);
      } else if (o == 3) { sColor = s.At(sr.Max.X - x, sr.Max.Y - y);
      } else if (o == 4) { sColor = s.At(x, sr.Max.Y - y);
      } else if (o == 5) { sColor = s.At(y, x);
      } else if (o == 6) { sColor = s.At(y, sr.Max.Y - x);
      } else if (o == 7) { sColor = s.At(sr.Max.X - y, sr.Max.Y - x);
      } else if (o == 8) { sColor = s.At(sr.Max.X - y, x);
      } else {
        sColor = s.At(x, y);
      }
      r, g, b, _ := sColor.RGBA()
